...
export const useAdRanking = (contentId?: string) => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: context } = await supabase.from('content_context').select('*').eq('content_id', contentId).maybeSingle();

      // 2. Get user interests.
      const { data: interests } = await supabase.from('ads_user_interests').select('topic_id, interest_score').eq('user_id', user.id);
